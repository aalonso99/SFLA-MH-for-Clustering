...
void sfla_searchv1(const string& dataset, 
    ...
    const unsigned N_MEMEPLEXES = 10;
    const unsigned FROGS_PER_MEMEPLEX = 30;
    const unsigned FROGS_PER_SUBMEMEPLEX = 10;
    const unsigned ITERS_IN_MEMEPLEX = 20;
    ...
    do{
        ...
        for(unsigned j=0; j<N_MEMEPLEXES; j++){
            for(auto it=memeplexes[j].begin(); it!=memeplexes[j].end(); it++){
                loss.insert(*it);
            }
        }

    }while( f_objetivo_evals<MAX_EVALS );
