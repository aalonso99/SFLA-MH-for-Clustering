...
void sfla_searchv2_2parallel(const string& dataset, 
    ...
    for(unsigned rep=0; rep<N_REINICIOS; rep++){
            ...
            #pragma omp parallel num_threads(4)
            {
                ...
                for(unsigned i=0; i<N_MEMEPLEXES; i++){     //Dentro de cada memeplex...
                    for(unsigned j=0; j<ITERS_IN_MEMEPLEX; j++){     //Para cada iteración en cada memeplex...
                        ...
                        //Como el map ordena internamente sus elementos, asumo que esto debe modificar el orden en el memeplex
        ...
        if( rep<(N_REINICIOS-1) ){
            ...
            auto it = loss.begin();
            advance(it, 2*(rep+1));
            set<pair<unsigned,float>, compare_frogs> nuevo_loss(loss.begin(), it);
            #pragma omp parallel shared(nuevo_loss, loss, frogs, centroides) num_threads(4)
            {
                auto loss_it = loss.begin();
                advance( loss_it, omp_get_thread_num()+rep+1 );
                int m=omp_get_thread_num();
                while( loss_it!=loss.end() ){
                    unsigned idx = loss_it->first;
                    for(int i=0; i<n_clust; i++){
                        for(int j=0; j<dim; j++){
                            centroides[idx][i][j] = random_float(0,1)*(vec_max[j]-vec_min[j])+vec_min[j];
                        }
                    }
                    //Selección del cluster asignado a cada elemento de cada rana (se toma el centroide más cercano)
                    for(int i=0; i<size; i++){
                        float dist = FLT_MAX;
                        int mejor = 0;
                        for(int k=0; k<n_clust; k++){
                            float dist_cluster_k = distancia( dat[i].data(), centroides[idx][k], dim );
                            if( dist_cluster_k < dist ){
                                mejor = k;
                                dist = dist_cluster_k;
                            } 
                    ...
                    if( any_empty_cluster(n_clust, frogs[idx].data(), size) ){
                    ...
                    float obj = f_objetivo(frogs[idx].data(), dat, 
                                           centroides[idx], n_clust, size, dim, L, lista_rest);
                    #pragma omp critical
                    {   
                        nuevo_loss.insert( make_pair( idx, obj ) );
                    }
                    auto prev = loss_it->first;
                    advance(loss_it, omp_get_num_threads());
                    if(loss_it->first==prev){
                        loss_it++;
                    }
                }
            }
